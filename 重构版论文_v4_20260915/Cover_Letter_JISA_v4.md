...
**Reproducibility.** Code, per-row predictions, audit intermediates, the ten-seed run, the gate search, the identifiability analysis and the expert-diversity suite are released at https://github.com/linran-muxue/leakage-controlled-nids-study (release v1.10.0, tag v1.10.0). Raw datasets are not redistributed; provenance, retrieval dates and SHA-256 checksums are recorded. A 118-test suite runs in continuous integration.
