...
Placing the five magnitudes side by side: aggregation 0.0005, feature view +0.0021, deduplication order +0.0014 (maximum 0.0060), tuning budget +0.0078 and class prior +0.0725. **H3 is falsified: protocols are not interchangeable, and protocol effects outweigh model effects.**
...
Processing scripts, audit intermediates, per-row predictions and figure-generation code are released at https://github.com/linran-muxue/leakage-controlled-nids-study (release v1.1.0, tag v1.1.0). Raw datasets are not redistributed; the paper records source URLs, retrieval dates, version snapshots and SHA-256 checksums.
