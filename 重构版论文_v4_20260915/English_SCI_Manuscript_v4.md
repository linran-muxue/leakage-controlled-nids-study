...
Fused probabilities are then temperature-scaled on the validation partition [27], and a Mondrian (class-conditional) conformal predictor provides an optional `unknown` output at significance level alpha = 0.1. [30-33] The mechanism changes probabilities first; a hard label changes only if the fused argmax changes. "The probabilities moved" and "the prediction changed" are therefore distinct statements, and Sections 5.2 and 5.3 report them separately.
