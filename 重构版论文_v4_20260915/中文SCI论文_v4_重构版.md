...
3. **一张把协议效应与模型效应分离的定量地图**。我们在同一批数据上测量四类差异来源并给出量级：聚合策略（0.0005）、特征视图（+0.0021）、类别先验（+0.0725）、去重顺序（至多 +0.0060）、文件外推（0.33–1.00）。结论是协议效应比模型效应高出一个数量级。
...
第三，**协议效应主导模型效应。** 在同一批实验中，类别先验变化带来 +0.0725 的 Macro-F1 差异，去重顺序至多 +0.0060，而聚合策略仅 0.0005。文件级压力测试中，同一模型的 Macro-F1 跨度达 0.3325 至 0.9997。因此，在公开入侵检测数据集上，**报告什么协议比报告什么模型更能决定结论**。
