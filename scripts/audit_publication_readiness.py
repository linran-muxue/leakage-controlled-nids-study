...
def main() -> None:
    cic = ROOT / "data_processed_cic_natural_v3b"
    nsl = ROOT / "data_external_nsl_kdd_processed_v2"
    tuned = ROOT / "results_rccf_cic_natural_v3b"
    ext = ROOT / "results_rccf_cic_natural_v3b"
    ...
    main_metrics = pd.read_csv(tuned / "metrics_by_seed.csv")
    ...
    rf = main_metrics[main_metrics.model.astype(str).str.contains("rccf", case=False, na=False)]
    wrf = ext_metrics[ext_metrics.model.astype(str).str.contains("rccf", case=False, na=False)]
    ...
    lines = [
        ...
        f"- v3b 主实验为五分类自然先验研究总体，共 {audit['capped_rows_before_balance']:,} 条；另有平衡控制子集 {audit['balanced_rows']:,} 条。自然协议训练/验证/测试为 {splits['train']['rows']}/{splits['validation']['rows']}/{splits['test']['rows']}。",
