...
md = root / 'results_paper_materials_v3' / 'english_sci_manuscript_final.md'
...
for f in [root/'results_paper_materials_v3/english_sci_manuscript_final.docx', root/'results_paper_materials_v3/Highlights_JISA.docx']:
    if not f.exists():
        errors.append(f'missing file: {f.name}')
if errors:
    raise SystemExit('JISA_AUDIT_FAIL: ' + '; '.join(errors))
doc = Document(root/'results_paper_materials_v3/english_sci_manuscript_final.docx')
