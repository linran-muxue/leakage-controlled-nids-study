...
def main():
    ...
    pd.DataFrame(rows).to_csv(args.output_dir/"outer_modelwise_metrics.csv",index=False,encoding="utf-8-sig"); pd.DataFrame(tune_rows).to_csv(args.output_dir/"inner_modelwise_tuning.csv",index=False,encoding="utf-8-sig"); pd.DataFrame(rows).groupby("model")[["accuracy","balanced_accuracy","macro_f1","log_loss","brier_macro","ece"]].agg(["mean","std"]).reset_index().to_csv(args.output_dir/"outer_modelwise_summary.csv",index=False,encoding="utf-8-sig"); (args.output_dir/"protocol.json").write_text(json.dumps({"development_pool":"data_processed_cic_natural_v3b/train.csv + validation.csv","untouched_final_test":"data_processed_cic_natural_v3b/test.csv","canonical_processed_protocol":"data_processed_cic_natural_v3b","outer_splits":args.outer_splits,"inner_splits":args.inner_splits,"k_values":args.k_values,"modelwise_tuning":True},ensure_ascii=False,indent=2),encoding="utf-8")
